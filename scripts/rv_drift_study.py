...
HORIZONS = [5, 10, 20]
PCT_BINS = [0.0, 0.1, 0.3, 0.7, 0.9, 1.0]
PCT_LABELS = ["0-10%", "10-30%", "30-70%", "70-90%", "90-100%"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RV percentile drift study.")
...
def main(days: int, window: int, save: bool, dsn: str = "") -> None:
    ...
    # Rolling percentile rank (backward-looking only)
    rv = df["rv48"]
    pct_rank = rv.rolling(window).apply(lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)
    df["rv_pct"] = pct_rank
    df["rv_bucket"] = pd.cut(df["rv_pct"], bins=PCT_BINS, labels=PCT_LABELS, right=False)
    ...
    if save:
        ...
        out.to_csv(f"logs/rv_drift_{ts}.csv", index=False)
        with open("logs/summary.log", "a", encoding="utf-8") as f:
            f.write(f"[{ts}] rv_drift days={days} rows={len(out)} window={window}\n")
