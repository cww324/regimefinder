...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Deterministic non-live H32 research runner.")
...
def build_h32_events(days: int, horizon: int, dsn: str = "") -> pd.DataFrame:
    # This mirrors the H32 kickoff inline block logic from FINDINGS_TECHNICAL.md.
    if dsn:
    ...
    idx = dedup_idx(x["entry_h32"], horizon)
...
def main() -> None:
    ...
    if args.timeframe != "5m":
        raise ValueError("research_h32_runner supports timeframe=5m only for frozen H32.")
    if args.horizon != 6:
        raise ValueError("Frozen H32 runner expects horizon=6.")

    mode = args.cost_mode
    cost = cost_value(mode)
    train_days, test_days, step_days = [int(x) for x in args.wf]

    events = build_h32_events(days=int(args.days), horizon=int(args.horizon), dsn=args.dsn.strip())
    ...
    print(f"H32_RUNNER {mode} n={b['n']} mean={b['mean']:+.6f} ci=[{b['mean_ci_low']},{b['mean_ci_high']}] p={b['p_mean_gt_0']}")
    print(f"H32_WF {mode} n={wf['aggregate']['n']} mean={wf['aggregate']['mean']:+.6f} ci=[{wf['aggregate']['mean_ci_low']},{wf['aggregate']['mean_ci_high']}] p={wf['aggregate']['p_mean_gt_0']}")
